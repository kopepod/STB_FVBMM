function [labels, A] = nAgekELM(train_data, test_data, train_lbls, C, n, m, clst_bool)
...
if clst_bool~=1
    ...
    if N > 100000
        ...
        [tmp_vec, ref_data] = fkmeans(double(train_data'), n, options);
    end
...
[max_vals,tr_lbls] = max(T);
%LDFA
%[Slda_w, Slda_b] = matrices_LDA2(K_Nn, tr_lbls);
[Slda_w, Slda_b] = matrices_LFDA(K_Nn, tr_lbls);
%W = pinv( K_Nn*K_Nn' + (eye(n) + m*pinv(Slda_b+0.01*eye(n))*Slda_w)/C ) * K_Nn * T'; 
W = ( ( K_Nn*K_Nn' + (eye(n) + m*pinv(Slda_b+0.01*eye(n))*Slda_w)/C ) \ K_Nn ) * T'; 
