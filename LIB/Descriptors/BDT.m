classdef BDT
 ...
 properties(Constant = true)
  bn = cos(0:2*pi/12:2*pi-2*pi/12) + 1j * sin(0:2*pi/12:2*pi-2*pi/12);
   ...
   fields = BDT.getRegions([32,15]);
   yx = BDT.xytSpV(32,32,1);
 end
 ...
 methods(Static = true)
  ...
  function code = binTraj(x,y)
   ...
   for k = 1:numel(s)
    ...
    if s(k) > 0.2 && s(k) < 0.8
     [~,idx] = min(bsxfun(@minus,u(k),BDT.bn));
     code{k} = BDT.bs{idx};
     continue
    end
  ...
  function regions = getRegions(st)
   ...
   Y = BDT.splitInt(size(v,1),4);
   X = BDT.splitInt(size(v,2),4);
   T = BDT.splitInt(size(v,3),2);
   
   v = mat2cell(v,Y,X,T);
   
   v = BDT.fillvolumeidx(v);
